...
void sample_bilinear(float u, float v, float* smpl, Buffer* buf)
{
    ...
    u = clampf(u, 0.0f, 1.0f); // ROBUSTNESS
    v = clampf(v, 0.0f, 1.0f);

    assert(buf->fpp <= MAX_FPP);
    float tl[MAX_FPP], tr[MAX_FPP], bl[MAX_FPP], br[MAX_FPP];

    // ASSUMPTION: no 1xn or 1xm or 1x1 buffers (could clamp it, but I most likely won't be having this problem)
...
void blit_buffer(Buffer* src, Buffer* target, float x_offset, float y_offset, float width_percent, float height_percent)
{
    ...
    int top_right[2]   = { minf(x_offset + target->width * width_percent, target->width), minf(y_offset + target->height * height_percent, target->height) };
